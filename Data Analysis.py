...
def aggregateTrials(tables):
    ...
    for frame in tables:
        if str(tables[frame].iat[0, 0]) not in trialsList:
            trialsList[str(tables[frame].iat[0, 0])] = [tables[frame]]
        else:
            trialsList[str(tables[frame].iat[0, 0])].append(tables[frame])
            
    for experiment in trialsList:
        experiments['Experiment ' + str(experiment)] = pd.concat(trialsList[experiment], ignore_index=True)
...
def importData(file):
        dataFrames = {}
        dataFrames['Experiment 2'] = pd.read_csv(file)
        return dataFrames
...
def cleanData(dataFrames):
    for frame in dataFrames:
        ...
        temp = dataFrames[frame]
        # 
        
        # for index, row in temp.iterrows():

        
        #groups = temp.groupby('pos')
        
        #for name, group in groups:
            #if name == 12:
                #group[]
        
        # mask = temp.isin({'pos': [12]}).all(1)
        # temp[mask, ' Remote_Bluetooth_Address':] = temp[mask, ' Scan_Mode':' bat_end_current_capacity']
        
        #temp1.to_csv('fixed rows.csv')
    
    return dataFrames

def shiftPos10(row):
    if row['pos'] == 12:
        return row[:13] + row[12:-1]
    else:
        return row
    #[' Remote_Bluetooth_Address':][' Scan_Mode':' bat_end_current_capacity']
...
def dataAnalysis(dataFrames):
    ...
    rssiResults = tempRSSI = analyzeRSSI(dataFrames['Experiment 2'])
    discoveryResults = analyzeInitialDiscovery(dataFrames['Experiment 2'])
    
    captureStats = pd.concat([percentCaptureResults, countResults, rssiResults, discoveryResults], axis=1)
...
def plotStats(frame):
    ...
    variables = ['Time_to_Discovery', 'RSSI', 'Capture_Count', 'Percent_Captures']
...
if __name__ == "__main__":
    ...
    experiments = importData('Raw_Data.csv')
